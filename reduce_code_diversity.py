#!/usr/bin/env python3
"""
Aggregate a code_diversity_summary.csv file by averaging the last N generations.

Example:
    python reduce_code_diversity.py deepseek-60-focus/code_diversity_summary.csv \
        --generations 3 --output deepseek-60-focus/code_diversity_summary_last3.csv
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Average the last N generations of a code diversity summary CSV."
    )
    parser.add_argument(
        "input_csv",
        type=Path,
        help="Path to code_diversity_summary.csv produced by code_diversity_quantifier.py.",
    )
    parser.add_argument(
        "--generations",
        type=int,
...
def validate_columns(columns: Iterable[str]) -> None:
    missing = [col for col in DISTANCE_COLUMNS if col not in columns]
    if missing:
